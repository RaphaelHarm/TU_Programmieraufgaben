...
import numpy as np
from gui import QtGui, QtWidgets, Ui_MainWindow
...
class App(Ui_MainWindow):
    ...
    def load_model(self):
        ...
        path = QtWidgets.QFileDialog.getOpenFileName(
            self.window, directory="./data/", filter="*.ply"
        )[0]
        print(path)
    ...
    def rasterize(self):
        ...
        height, width, channel = self.img.shape  # type: ignore
        bytesPerLine = 3 * width
        qImg = QtGui.QImage(  # type: ignore
    ...
    def save_image(self):
        ...
        path = path = QtWidgets.QFileDialog.getSaveFileName(
            self.window,
            directory=f"./saved_images/{self.model.model_name}_{self.rasterization_mode}.png",
            filter="*.png",
        )[0]
        if path is None or path == "":
            return
