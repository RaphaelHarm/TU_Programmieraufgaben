...
class Framebuffer:
    def __init__(self, height: int, width: int):
        """
        Initializes a framebuffer of size (height width 3) and a zbuffer to save
        the depth values.
        """
        self.width = width
        self.height = height
    ...
    def set_pixel(
        ...
        """
        Sets the corresponding pixels at location (x,y) to the color values and
        the zbuffer.
        """
        ...
        if not len(new_coords):
            return  # if we have no coordinates to set, it should not raise an error (but maybe a warning or some kind of information?)
