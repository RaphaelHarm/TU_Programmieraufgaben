...
def fill_rasterization(mesh: Mesh, framebuffer: Framebuffer):
    """
...
def line_eq(
    A: float, B: float, C: float, x: float | np.ndarray, y: float | np.ndarray
) -> float | np.ndarray:
    """
    Defines the line equation described by the provided parameters and returns
    the distance of a point (x, y) to this line.

    Args:
        A: Line equation parameter 1.
        B: Line equation parameter 2.
        C: Line equation parameter 3.
        x: x coordinate(s) of point to test against the line.
        y: y coordinate(s) of point to test against the line.

    Returns:
        The distance of the point (x, y) to the line (A, B, C).
    """
    ### STUDENT CODE
    # TODO:     [TODO3] Implement this function.
...
def draw_triangle(
    framebuffer: Framebuffer, v1: MeshVertex, v2: MeshVertex, v3: MeshVertex
):
    """
